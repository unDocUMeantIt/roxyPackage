...
\begin{abstract}
R packages consist of numerous files in various formats, like \texttt{*.R}, \texttt{*.Rd}, \texttt{DESCRIPTION}, \texttt{NAMESPACE} or \texttt{CITATION}, ordered in a specific directory structure. \pckg{} tries to automatically generate most of these structures and files, so that you can concentrate on writing the actual R code for your package.
If your code includes \texttt{roxygen2}-style documentation, this package can update your docs as well. It is also capable of creating a local package repository, including limited support for generating Mac OS X, Windows and Debian packages.
\end{abstract}
...
\begin{itemize}
  \item{where is the source directory of the package (\texttt{"\textasciitilde/myRcode/examplePackage"})} 
  \item{where is the R library the package can be installed to (e.\,g., \texttt{"\textasciitilde/R"})}
  \item{in which directory should the repository be created (e.\,g., \texttt{"\textasciitilde/myRcode/repo"})}
...
\begin{description}
  ...
  \item{\texttt{"readme":}}{ Generate initial \texttt{README.md} file (optionally including a Flattr button and GitHub URL).}
...
\section{No more excuses: Generating template files}

Imagine you just wrote a new function or two and would like to share your work with others. But when you try to remember how to write sufficient \texttt{roxygen2} documentation, your motivation disappears. Well, \pckg{} comes to the rescue: It can write template files for you, prefilled with some usefull tags and hints which should leave you with no more excuses. Depending on the arguments given to \texttt{templateFile()}, you can see the script code on screen or write it directly to a file:
...
In addition tu documenting R functions, \texttt{templateFile()} can also produce useful templates for S4 classes and methods.
